...
const MainDashboard = () => {
    ...
    if (newClientName) {
      try {
        const response = await axios.patch(
          "https://us-central1-gatewayfunc.cloudfunctions.net/app/api/clients",
  ...
  const handleDeleteClient = async (clientName) => {
    try {
      const response = await axios.delete("https://us-central1-gatewayfunc.cloudfunctions.net/app/api/clients", {
        data: { clientName },
      });
      setProjects(response.data);
    } catch (error) {
  ...
  const fetchClientsCount = async () => {
    try {
      const response = await axios.get(
        "https://us-central1-gatewayfunc.cloudfunctions.net/app/api/clients/count"
      );
  ...
  const fetchExtrainfo = async () => {
    try {
      const response = await axios.get('https://us-central1-gatewayfunc.cloudfunctions.net/app/api/extrainfo2');
